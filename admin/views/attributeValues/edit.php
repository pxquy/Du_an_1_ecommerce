...
<form action="<?= BASE_URL_ADMIN . '&action=attributeValues-update&id=' . $attributeValue['id'] ?>" method="post"
    ...
    <div class="mb-3 mt-3">
        ...
        <select name="attributeId" id="attributeId">
            <?php foreach ($attributePluck as $id => $attributeName): ?>
                <option value="<?= $id ?>" <?= $attributeValue['attributeId'] == $id ? 'selected' : '' ?>><?= $attributeName ?>
                </option>
            <?php endforeach ?>
        </select>
    </div>
