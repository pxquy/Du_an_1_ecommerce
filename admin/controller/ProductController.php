...
class ProductController
{
    ...
    public function index()
    {
        ...
        $data = $this->product->select('*', '1 = 1 ORDER BY id ASC');
    ...
    public function store()
    {
        try {
            ...
            if (
                ...
                || !empty($this->product->find('*', 'email = :email', ['email' => $data['email']]))
            ) {
            ...
            if ($data['avatarUrl']['size'] > 0) {
                $data['avatarUrl'] = upload_file('products', $data['avatarUrl']);
            } else {
                $data['avatarUrl'] = null;
            }
    ...
    public function delete()
    {
        try {
            ...
            if ($rowCount > 0) {
                ...
                if (!empty($product['avatarUrl']) && file_exists(PATH_ASSETS_UPLOADS . $product['avatarUrlUrl'])) {
                    unlink(PATH_ASSETS_UPLOADS . $product['avatarUrlUrl']);
                }
